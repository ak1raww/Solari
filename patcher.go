...
func init() {
	if dir := os.Getenv("EQUICORD_USER_DATA_DIR"); dir != "" {
		...
		BaseDir = path.Join(dir, "..", "SolarcordData")
	} else {
...
func patchAppAsar(dir string, isSystemElectron bool) (err error) {
	...
	if err := WriteAppAsar(appAsar, SolarcordDirectory); err != nil {
		return err
	}
...
func (di *DiscordInstall) patch() error {
	...
	if di.isFlatpak {
		...
		Log.Debug("This is a flatpak. Trying to grant the Flatpak access to", SolarcordDirectory+"...")
		...
		args = append(args, "override", name, "--filesystem="+SolarcordDirectory)
		...
		if err != nil {
			return errors.New("Failed to grant Discord Flatpak access to " + SolarcordDirectory + ": " + err.Error())
		}
	}
	return nil
}
...
func isSolarcordLoaderAppAsar(appAsar string) (bool, error) {
...
func cleanupDesyncedPatchedInstall(dir string, isSystemElectron bool) (bool, error) {
	...
	isLoader, err := isSolarcordLoaderAppAsar(appAsar)
	if err != nil {
		return false, err
	}
	if isLoader {
	...
	Log.Warn("Detected a patched install with a non-Solarcord app.asar. Discord was likely updated while patched; removing stale _app.asar")
