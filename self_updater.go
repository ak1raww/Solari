...
func GetInstallerDownloadLink() string {
	...
	switch runtime.GOOS {
	case "windows":
		filename := Ternary(buildinfo.UiType == buildinfo.UiTypeCli, "SolariCli.exe", "Solari.exe")
		return BaseUrl + filename
	case "darwin":
		switch runtime.GOARCH {
		case "amd64":
			return BaseUrl + "Solari-darwin-x64.zip"
		case "arm64":
			return BaseUrl + "Solari-darwin-arm64.zip"
		default:
			return ""
		}
	case "linux":
