...
func main() {
	...
	var installFlag = flag.Bool("install", false, "Install Solarcord")
	var updateFlag = flag.Bool("repair", false, "Repair Solarcord")
	var uninstallFlag = flag.Bool("uninstall", false, "Uninstall Solarcord")
	...
	if *versionFlag {
		fmt.Println("Solari Cli", buildinfo.InstallerTag, "("+buildinfo.InstallerGitHash+")")
		fmt.Println("Copyright (C) 2026 Vendicated, thororen1234, Vencord, and Solarcord contributors")
	...
	if !SliceContainsFunc(switches, func(b *bool) bool { return *b }) {
		...
		go func() {
			...
			if IsSelfOutdated {
				...
				Log.Warn("To update, select the 'Update Solari' option to update, or run with --update-self")
			}
		}()
	...
	if install {
		...
		Log.Info("Downloading latest Solarcord files...")
...
func PromptDiscord(action, dir, branch string) *DiscordInstall {
	...
	items := SliceMap(discords, func(d any) string {
		...
		return fmt.Sprintf("%s - %s%s", strings.Title(install.branch), install.path, Ternary(install.isPatched, " [Equicord Installed]", ""))
	})
...
func HandleScuffedInstall() {
	...
	fmt.Println("Otherwise, Solarcord will likely not work.")
}