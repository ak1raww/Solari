...
func InitGithubDownloader() {
	...
	IsDevInstall = os.Getenv("SOLARCORD_DEV_INSTALL") == "1"
	...
	SolarcordFile := SolarcordDirectory

	stat, err := os.Stat(SolarcordFile)
	if err != nil {
		return
	}
	...
	// dev
	if stat.IsDir() {
		SolarcordFile = path.Join(SolarcordFile, "main.js")
	}

	// Check hash of installed version if exists
	b, err := os.ReadFile(SolarcordFile)
	if err != nil {
		return
	}

	Log.Debug("Found existing Solarcord Install. Checking for hash...")

	re := regexp.MustCompile(`// Solarcord (\w+)`)
...
func installLatestBuilds() (retErr error) {
	...
	out, err := os.OpenFile(SolarcordDirectory, os.O_WRONLY|os.O_CREATE|os.O_TRUNC, 0644)
	if err != nil {
		Log.Error("Failed to create", SolarcordDirectory+":", err)
		retErr = err
		return
	}
	read, err := io.Copy(out, res.Body)
	if err != nil {
		Log.Error("Failed to download to", SolarcordDirectory+":", err)
		retErr = err
		return
	}
	...
	_ = FixOwnership(SolarcordDirectory)
