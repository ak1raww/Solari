...
func renderInstaller() g.Widget {
	...
	layout := g.Layout{
		...
		InfoModal("#patched", "Successfully Patched", "If Discord is still open, fully close it first.\n"+
			"Then, start it and verify Solarcord installed successfully by looking for its category in Discord Settings"),
...
func loop() {
	...
	g.SingleWindow().
		...
		Layout(
			...
			g.Style().SetFontSize(20).To(
				g.Row(
					g.Label(Ternary(IsDevInstall, "Dev Install: ", "Solarcord will be downloaded to: ")+SolarcordDirectory),
					g.Style().
						SetColor(g.StyleColorButton, DiscordBlue).
						SetStyle(g.StyleVarFramePadding, 4, 4).
						To(
							g.Button("Open Directory").OnClick(func() {
								g.OpenURL("file://" + path.Dir(SolarcordDirectory))
							}),
						),
				),
				...
				g.Label("Local Solarcord Version: "+InstalledHash),
				&CondWidget{
					GithubError == nil,
					func() g.Widget {
						if IsDevInstall {
							return g.Label("Not updating Solarcord due to being in DevMode")
						}
						return g.Label("Latest Solarcord Version: " + LatestHash)
					}, func() g.Widget {
